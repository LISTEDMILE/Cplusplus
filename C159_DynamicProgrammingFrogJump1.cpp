...
int findMinPathByRecursion(vector<int> &arr, int idx)
{ // O(n2) , space-O(n)
    if (idx == 0)
    {
        return 0;
...
int findMinPathOptimalMemoization(vector<int> &arr, int idx, vector<int> &p)
{ // O(n) , space-O(n+n)
    if (idx == 0)
    {
        return 0;
...
int findMinPathOptimalTabular(vector<int> &arr, int n, vector<int> &p)
{ // O(n) , space-O(n)
...
int findMinPathMostOptimal(vector<int> &arr, int n)
{ // O(n) , space-O(1)
